...
class Angle
{
    ...
    public:
     Angle(int de,float m,char di):degree(de),min(m),direction(di)
     {}

     void get_details()
     {
         cout<<"enter the degrees and minutes"<<endl;
         cin>>degree>>min;
     ...
     void display()
     {
         cout<<degree<<'\xF8'<<min<<"\' "<<direction<<endl;// \xF8 prints out degree symbol
     }
...
int main()
{
    Angle a1(0,0.0,'N');//questn says to initialize the object at some random angle and
    a1.display();//also display it
    char choice='y';
    while(choice=='y')//user can keep entering the angles unyil he wants 
    {
        a1.get_details();
        a1.display();
